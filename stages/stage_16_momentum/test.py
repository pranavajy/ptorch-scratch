...
If stage_08 / stage_13 / stage_16 are not implemented yet, the suite skips
cleanly instead of erroring, so you can run it incrementally.

Run with:  pytest stage_16_momentum/test.py
"""
...
# ``Momentum`` is this stage's ``SGDMomentum`` (a subclass of stage_13's SGD);
# ``SGD`` comes straight from stage_13 so the tests compare the two directly.
try:
    from dlfs import stage_import

    Stage16_Momentum, Stage16_Tensor, Stage16_quadratic_descent = stage_import(
        "stage_16", "Momentum", "Tensor", "quadratic_descent"
    )
    Stage13_SGD = stage_import("stage_13", "SGD")
    # Local short names for readability in the test body below.
    Tensor = Stage16_Tensor
    Momentum = Stage16_Momentum
    quadratic_descent = Stage16_quadratic_descent
except (ImportError, NotImplementedError) as exc:  # pragma: no cover
    pytest.skip(
        f"stage_16 Momentum / stage_13 SGD / stage_08 Tensor not importable "
        f"yet: {exc}",
...
def test_beta_zero_equals_plain_sgd():
    """With beta=0, Momentum must match stage_13's SGD step-for-step."""
    A, b, x0 = quad_problem()
...
def test_momentum_converges_faster_than_sgd():
    ...
    hist_sgd = quadratic_descent(lambda p: Stage13_SGD([p], lr), x0.copy(), A, b, steps)
