...
stage_08. ``Neuron`` lives in this stage's ``code.py`` and is built on the
``Tensor`` imported from stage_08 through ``dlfs.stage_import``. If stage_08 or
stage_09's ``Neuron`` is not yet implemented, the suite skips rather than
erroring, so you can run it incrementally.

Run with:  pytest stage_09_neuron/test.py
"""
import os as _os
import sys as _sys
...
try:
    ...
    pytest.skip(
        f"stage_09 Neuron / stage_08 Tensor not importable yet: {exc}",
