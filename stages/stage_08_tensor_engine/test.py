...
# --------------------------------------------------------------------------- #
# subclass propagation: every op must build its result via `type(self)(...)`
# --------------------------------------------------------------------------- #
# Later stages SUBCLASS Tensor to override an op (stage_11 swaps in a
# broadcasting-correct `__add__`/`__mul__`). For that override to take effect
# through a CHAIN of ops, each op must construct its result node with the
# *runtime* class -- `type(self)(...)`, not a hardcoded `Tensor(...)`. If any op
# hardcodes `Tensor(...)`, then e.g. `(a + b)` on two subclass tensors returns a
# base Tensor and `(a + b).relu()` keys on the base type, silently dropping the
# subclass override. This test is the canary: a trivial no-behaviour-change
# subclass must survive every op, the coerced-constant path, and the classmethod
# constructors.
#
# xfail (non-strict) so the unsolved stub -- where ops raise NotImplementedError
# -- is reported xfail rather than red; a correct solution xpasses.
@pytest.mark.xfail(raises=NotImplementedError, strict=False,
                   reason="ops must build via type(self)/cls; xfails on the stub")
def test_ops_preserve_subclass():
    class Sub(Tensor):
        """Trivial subclass: no overrides, just a distinct runtime class."""
        pass

    a = Sub([1.0, 2.0, 3.0])
    b = Sub([4.0, 5.0, 6.0])

    # binary ops, derived ops, and reflected/coerced-constant operands
    assert type(a + b) is Sub, "__add__ must build via type(self)(...)"
    assert type(a * b) is Sub, "__mul__ must build via type(self)(...)"
    assert type(a - b) is Sub, "__sub__ (derived) must stay subclass"
    assert type(a / b) is Sub, "__truediv__ (derived) must stay subclass"
    assert type(-a) is Sub, "__neg__ (derived) must stay subclass"
    assert type(a + 2.0) is Sub, "coerced raw constant must become the subclass"
    assert type(2.0 + a) is Sub, "__radd__ must stay subclass"

    # unary / elementwise
    assert type(a ** 2) is Sub, "__pow__ must build via type(self)(...)"
    assert type(a.relu()) is Sub
    assert type(a.tanh()) is Sub
    assert type(a.exp()) is Sub
    assert type(a.reshape(3)) is Sub

    # matmul keeps the subclass too
    M = Sub([[1.0, 0.0], [0.0, 1.0], [1.0, 1.0]])  # (3,2)
    assert type(a @ M) is Sub, "__matmul__ must build via type(self)(...)"

    # a CHAINED graph: the override-carrying class must survive every hop
    chained = ((a + b) * 2.0).relu() ** 2
    assert type(chained) is Sub, "subclass must propagate through chained ops"

    # classmethod constructors build via cls(...)
    assert type(Sub._coerce(5.0)) is Sub, "_coerce must wrap via cls(...)"
