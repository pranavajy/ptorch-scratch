...
Depends on stage_08 (Tensor), stage_10 (Dense), stage_24 (Conv2D/MaxPool2D/
Flatten), and the stage_31 mytorch API (cross_entropy_loss / Adam / DataLoader /
Dataset, re-exporting stages 13 / 18 / 20). If any is not implemented yet, the
suite skips cleanly.

Run with:  pytest stage_33_capstone_cifar10/test.py
"""
import os as _os
import sys as _sys
...
try:
    ...
    pytest.skip(
        f"stage_33 capstone or a prior dependency "
        f"(stage_08/11/13/18/20/25/26) not importable yet: {exc}",
        allow_module_level=True,
    )

# Load DataLoader / Dataset through the shared dlfs shim (stage_31 re-exports the
# stage_19 plumbing; the code module imports them the same way).
try:
    from dlfs import stage_import

    Stage31_DataLoader, Stage31_Dataset = stage_import("stage_31", "DataLoader", "Dataset")
except (ImportError, NotImplementedError):  # pragma: no cover
    Stage31_DataLoader = None
    Stage31_Dataset = None
...
def test_train_cifar_converges_and_records_lr():
    ...
    epochs. Skipped if stage_19's DataLoader is unbuilt.
    """
    if Stage31_DataLoader is None or Stage31_Dataset is None:
        pytest.skip("stage_19 DataLoader/Dataset not importable yet.")

    X, y = skip_if_unbuilt(
        make_cifar_like, n_per_class=24, img_size=16, n_classes=3,
        noise=0.2, seed=3,
    )
    ds = skip_if_unbuilt(Stage31_Dataset, X, y)
    loader = skip_if_unbuilt(Stage31_DataLoader, ds, batch_size=12, shuffle=True, seed=3)
    model = skip_if_unbuilt(
