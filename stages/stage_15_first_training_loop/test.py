...
Run: pytest stage_14_first_training_loop/test.py
"""
import os as _os
import sys as _sys
...
# `accuracy` and re-exports `MLP` (stage_11), `mse_loss` (stage_12), `SGD`
# (stage_13), `Tensor` (stage_08), and `Dense`, all via dlfs.stage_import.
try:
    ...
    pytest.skip(
        f"stage_14 (or a dependency stage_08/12/13/14) not importable yet: {exc}",
...
# --------------------------------------------------------------------------- #
# helper: skip a test cleanly if a *dependency* (stage_08..14 / Dense) isn't
