...
- `class RMSProp(Optimizer)` — `RMSProp(params, lr=1e-2, beta=0.99, eps=1e-8, weight_decay=0.0)`:
  ...
  - `weight_decay` couples L2 into the gradient before the EMA: `g = p.grad + weight_decay * p.data` (same coupled convention as Adam below; AdamW decouples it).
...
- `class AdamW(Adam)` — same signature; **decoupled** weight decay: do the plain Adam update on `g` (no `weight_decay` in `g`), then additionally `p.data -= lr * weight_decay * p.data` (perform the decay as a separate subtraction after the adaptive update).
