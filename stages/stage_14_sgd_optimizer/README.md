...
**Imports + adds** — Imports `Tensor` from `stage_08` (via `dlfs.stage_import`) and uses it unchanged; ADDS a new `Optimizer` base class + `SGD` subclass — the optimizer contract later stages (momentum in `stage_17`, RMSProp/Adam/AdamW in `stage_18`) extend.

**Context** — You now have a `Tensor` engine (`stage_08`, with matmul `@` and `tanh`/`exp`/`log`), broadcasting-correct backward (`stage_11`), and reductions `sum`/`mean` (`stage_12`), plus layers (`stage_10`'s `Dense`) that expose `.parameters()`. So far you have updated weights by hand: call `.backward()`, then write `p.data -= lr * p.grad` for each parameter inline. This stage factors that update out into a reusable **optimizer** object — the same `torch.optim.SGD` interface every later stage (the training loop, momentum in `stage_17`, RMSProp/Adam/AdamW in `stage_18`, CNNs, the Transformer) will plug into.
...
**Exercise** — Implement `class Optimizer` (base) and `class SGD(Optimizer)` in `code.py`, using **only** the `Tensor` engine (`stage_08`, imported via `stage_12`, its latest extension), NumPy (array math on `.data`/`.grad` only), and the stdlib. **No** PyTorch/autograd. Do not compute any gradient here — you only read `p.grad` that `Tensor.backward()` already filled.
