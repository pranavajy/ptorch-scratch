...
**Imports + adds** — Imports `Tensor` from `stage_08` (via `dlfs.stage_import`) and uses it unchanged; ADDS a new `Optimizer` base class + `SGD` subclass — the optimizer contract later stages (momentum/Adam in `stage_18`) extend.
