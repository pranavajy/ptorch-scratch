...
(cross_entropy_loss), stage_17 (Adam), stage_19 (DataLoader/Dataset) and
stage_24 (Conv2D/MaxPool2D/Flatten). If any of those is not implemented yet, the
suite skips cleanly instead of erroring.

Run with:  pytest stage_25_cnn_project/test.py
"""
import os as _os
import sys as _sys
...
try:
    ...
    pytest.skip(
        f"stage_25 CNN or a prior dependency (stage_08/11/13/18/20/25) "
        f"not importable yet: {exc}",
        allow_module_level=True,
    )

# Load the DataLoader / Dataset from stage_19 via the shared dlfs shim (the same
# mechanism the module under test uses), so we can build loaders in the
# convergence test.
try:
    from dlfs import stage_import

    Stage19_DataLoader, Stage19_Dataset = stage_import(
        "stage_19", "DataLoader", "Dataset"
    )
except (ImportError, NotImplementedError):  # pragma: no cover
    Stage19_DataLoader = None
    Stage19_Dataset = None
...
def test_train_cnn_converges():
    ...
    accuracy within a few epochs. Skipped if stage_19's DataLoader is unbuilt.
    """
    if Stage19_DataLoader is None or Stage19_Dataset is None:
    ...
    ds = skip_if_unbuilt(Stage19_Dataset, X, y)
    loader = skip_if_unbuilt(
        Stage19_DataLoader, ds, batch_size=16, shuffle=True, seed=3
    )
    model = skip_if_unbuilt(
