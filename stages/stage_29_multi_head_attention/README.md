...
Imports `Tensor` (`stage_08`) and the single-head `SelfAttention` + `causal_mask` (`stage_28`); adds `MultiHeadAttention` (alias `MHA`) that holds `h` `SelfAttention` heads of width `d_k = d_model // h`, concatenates their outputs, and applies a learned `W_o` projection. No attention math is rewritten — every gradient flows through `Tensor.backward()`.
