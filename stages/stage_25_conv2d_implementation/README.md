...
**Layout note** — `stage_24` used `W_row` of shape `(C_out, C_in·kH·kW)` with `Y = X_col @ W_row.T`; this stage flips to `W_r` of shape `(C_in·kh·kw, C_out)` with `Y = cols @ W_r` — same math, transposed convention. Also note this stage's `im2col` returns a `(cols, x_padded_shape)` **tuple** (`col2im` needs the padded shape), and `Conv2D` here has no `dilation` (`stage_24`'s formulas cover it; the layer keeps stride/pad only).
